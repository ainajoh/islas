...
########################################
# RETRIEVE
dmet = DATA(data_domain=data_domain, param_SFC=param_SFC, param_ML=param_ML, fctime=[0, lt], modelrun=modelruntime)
dmet.retrieve()
...
def point_calculation(jindx, iindx):
    ...
    BL = point_alt_sl2pres(jindx, iindx, h, geotoreturn, t_v_level, p, dmet.surface_air_pressure,
                           dmet.surface_geopotential)
...
def meteogram_vertical(jindx, iindx, ax2):
    ...
    CS = ax2.contour(tx, p[:, :, jindx, iindx], theta[:, :, jindx, iindx], colors="black", levels=lvl, zorder=900)
    #################################
    # P2:HUMIDITY
    #################################
    ...
    CL = ax2.contour(tx, p[:, :, jindx, iindx], dtdz[:, :, jindx, iindx], levels=lvl, colors="gray", linewidth=1,
                     zorder=100, )
    ...
    ax2.legend([Z[0], P_BL[0], custom_lines[0], custom_lines[1], custom_lines[2]],
               ['Zeppelin = 479m', 'PBLH', 'Unstable', 'Very Stable', 'Stable'], loc='upper left').set_zorder(99999)
...
def meteogram(jindx, iindx, axm1, axm2, axm4):
    ...
    Q_gust = axm2.quiver(time_normal, wspeed_gust, dmet.x_wind_gust_10m[:, 0, jindx, iindx] / wspeed_gust,
...
def mapwithdata(point, dirName_b2, figname_b2):
    ...
    param_SFC = ["x_wind_10m", "y_wind_10m",
                 "air_temperature_2m", ]  # ["air_temperature_2m", "surface_air_pressure", "air_pressure_at_sea_level",
    ...
    for t in range(0, np.shape(dmap.time)[0]):
        ...
        CFW = plt.contourf(x, y, dmap.air_temperature_2m[t, 0, :, :] - 273.15, zorder=10, alpha=0.9, cmap=cmap)
        ...
        for p in point:
            ...
            points = plt.plot(x, y, marker='.', markersize=6.0, markeredgewidth=4,
                              markerfacecolor='red', markeredgecolor='red', zorder=1000, linestyle='None')
            figname_b2_2 = figname_b2_1 + "_LOC" + str(ip) + "[" + "{0:.2f}_{1:.2f}]".format(dmet.longitude[p],
                                                                                             dmet.latitude[p])
...
def each_point(point, dirName_b1, figname_b1, ip):  # figsize=(12, 14)figsize=(5, 7)
    ...
    # figm1.tight_layout()
    plt.savefig(dirName_b1 + figname_b1 + "_LOC" + str(ip) + "[" + "{0:.2f}_{1:.2f}]".format(dmet.longitude[point],
                                                                                             dmet.latitude[
                                                                                                 point]) + ".png")
    print("saved")
    # plt.show()
...
def meteogram_average(indx):
    ...
    w = axma2.quiver(time_normal, wspeed_mean, wspeed_meanx[:] / wspeed_mean, wspeed_meany[:] / wspeed_mean, scale=70,
                     zorder=2, color="r")
    ...
    w_g = axma2.quiver(time_normal, wspeed_g_mean, wspeed_g_meanx[:] / wspeed_g_mean, wspeed_g_meany[:] / wspeed_g_mean,
                       scale=70, zorder=2, color="r")
