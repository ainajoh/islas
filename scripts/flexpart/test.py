...
def set_background():
    ...
    m.drawmeridians(meridians, color=[55. / 255., 55. / 255., 55. / 255.], labels=[0, 0, 0, 1], linewidth=0.1,
                    fontsize=7)
    return m
...
def plot_with_cartopy(rainout_trajs):
    ...
    crs = ccrs.Stereographic(central_longitude=-10, central_latitude=65, true_scale_latitude=65)
...
def correct_for_basemap_trouble(trajs):
    ...
    idx_Border = np.where(
        (trajs["lon"] < -160) | (trajs["lon"] > 160))  # | (trajs["lon"]>175 ) ) #idx of traj crossing boundary
    # upidx_Border = np.where(trajs["lon"]>160 )
    idx_exlBorder = np.where((np.min(trajs["lon"], axis=1) >= -160) & (
                np.max(trajs["lon"], axis=1) <= 160))  # all traj excluding the ones passing -179

    # Gather all traj excluding the one that crosses the border.
    trajs_coorected = Tra()
    trajs_coorected.set_array(
        trajs[idx_exlBorder[0], :])  # make a copy of full array only with index that fulfill creataria
...
def traj_under700(t, file_in, trajs_main):
    ...
    rainout_index = np.where(
        ((trajs["Q"][:, 1] - trajs["Q"][:, 0]) > 0.) & (np.min(trajs["p"][:, 0:6], axis=1) < 700))  # 7592

    rainout_trajs = Tra()
    pickingsec = rainout_index[0]  # [::241*10]
    rainout_trajs.set_array(trajs[pickingsec, :])  # make a copy of full array only with index that fulfill creataria
    # --------------------------------------------------------------------
...
def traj(t, file_in, trajs_main):
    ...
    trajs = trajs_main[:, :].copy()
    print("traj read")
    rainout_index = np.where(((trajs["Q"][:, 1] - trajs["Q"][:,
                                                  0]) > 0.))  # &  (np.min(trajs["lat"], axis=1) < 35 ) & (np.min(trajs["p"][:,0:6], axis =1)  < 700 )) #7592
