...
def set_variable2d(modelruntime, steps, lvl, xres, yres, model):
    ...
    variable3d_arome["air_temperature_ml"][
        "description"
    ] = "temperature on pressure sigmal levels"
    ...
    variable3d_arome["divergence_vertical"][
        "description"
    ] = "Non Hydrostatic divergence of vertical velocity: D = -g(w(i) -w(i-1))"
    ...
    variable3d_arome["x_wind_ml"][
        "description"
    ] = "U wind along x axis on pressure sigmal levels"
    ...
    variable3d_arome["y_wind_ml"][
        "description"
    ] = "V wind along y axis on pressure sigmal levels"
    ...
    variable3d_arome["specific_humidity_ml"][
        "description"
    ] = "specific humidity on pressure sigmal levels"
    ...
    variable3d_arome["turbulent_kinetic_energy_ml"][
        "description"
    ] = "Turbulent kinetic energy on pressure sigmal levels"
    ...
    arome3d = check_data(
        date=modelruntime, model=model, param=param3d_arome, m_level=lvl
    )
    file_arome3d = arome3d.file
    print(file_arome3d)
    dmap_arome3d = get_data(
        model=model,
        file=file_arome3d,
        param=param3d_arome,
        step=steps,
    ...
    dmap_sfx2d = get_data(
        model=model, file=file_sfx2d, param=param2d_sfx, step=steps, date=modelruntime
    )
    dmap_sfx2d.retrieve()
    ...
    for t in np.arange(np.min(steps), np.max(steps) + 1, 1):
        ...
        validdate = (
            datetime.datetime(
                int(modelruntime[0:4]),
                int(modelruntime[4:6]),
                int(modelruntime[6:8]),
                int(modelruntime[8:10]),
            )
            + datetime.timedelta(hours=int(t))
        )
        ...
        attr["x_resolution"] = np.double(
            "2500.0"
        )  # np.double("2500.0")*xres  # Use: None
        ...
        attr["y_resolution"] = np.double(
            "2500.0"
        )  # np.double("2500.0")*yres   # Use: None
        ...
        for param in param3d_arome:
            vid = ncid.createVariable(
                variable3d_arome[param]["name"], "f4", ("level", "Y", "X"), zlib=True
            )
        ...
        for param in param2d_arome:
            vid = ncid.createVariable(
                variable2d_arome[param]["name"], "f4", ("Y", "X"), zlib=True
            )
        ...
        for param in param2d_sfx:
            vid = ncid.createVariable(
                variable2d_sfx[param]["name"], "f4", ("Y", "X"), zlib=True
            )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--datetime", help="YYYYMMDDHH for modelrun", required=True, type=str
    )
    parser.add_argument(
        "--steps",
        default=[0, 65],
