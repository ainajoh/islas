...
def point(datetime, point_name, point_lonlat, number_grid=1):
    if point_name:
        if "cyclone.hpc.uib.no" in platform.node():
            ...
            file = "../githubclones/islas/weathervis/weathervis/data/sites.yaml"
        else:
            file = "../../weathervis/weathervis/data/sites.yaml"

        site = setup_site(point_name, cfg=file)
        point_lonlat = [site["lon"], site["lat"]]
        print(point_lonlat)

    # test nearest lonlat
    check_all = check_data(
        date=datetime, model="AromeArctic", param=["longitude"], step=1
    )
    ...
    print("NUMBER GRID")
    print(number_grid)
    closest_idx = nearest_neighbour_idx(
        point_lonlat[0],
        point_lonlat[1],
        dmap_meps.longitude,
        dmap_meps.latitude,
...
def make_inputfile(
    ...
    subdict = {
        "{date_input_for_flexpart}": datetime,
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--sim_direction", type=str, default="1", help="1:foreward, -1:backward"
    )
    ...
    make_inputfile(
        ...
        number_grid=args.number_grid,
        begin_YYYYMMDD=args.begin_YYYYMMDD,
        end_YYYYMMDD=args.end_YYYYMMDD,
        begin_HHMMSS=args.begin_HHMMSS,
        end_HHMMSS=args.end_HHMMSS,
        begin_rel_YYYYMMDD=args.begin_rel_YYYYMMDD,
        end_rel_YYYYMMDD=args.end_rel_YYYYMMDD,
        begin_rel_HHMMSS=args.begin_rel_HHMMSS,
        end_rel_HHMMSS=args.end_rel_HHMMSS,
        sim_direction=args.sim_direction,
        file=args.file,
        ZPOINT_1=args.ZPOINT_1,
        ZPOINT_2=args.ZPOINT_2,
    )
