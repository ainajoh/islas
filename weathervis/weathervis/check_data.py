...
def filter_param(file, param):
    ...
    if param:  # If a user param is given
        for i in range(0, len(file)):  # go through all files,
            ...
            if (
                all(param_bool) == False
            ):  # remove those files not having that parameter.
...
class check_data:
    def __init__(
        ...
        self.maxstep = (
            np.max(step)
            if step != None or type(step) != float or type(step) != int
            else step
        )
        ...
        try:
            ...
            print(e)
            print(
                "There might be problems with the server; check out https://status.met.no"
            )

        if (
            webcheck.status_code != 200
        ):  # SomeError(ValueError, f'Type not found: choices:{levtype}')
            SomeError(
                ConnectionError,
        ...
        if self.date == None and self.param == None:
            self.param = self.check_variable_all(
                self.model, self.numbervar, self.search
            )
            if self.search:
    ...
    def check_available_date(self, model, search=None):
        ...
        dfc = (
            df.copy()
        )  # df['base_name'] = [re.sub(r'_[0-9]*T[0-9]*Z.nc','', str(x)) for x in df['File']]
        drop_files = [
            "_vc_",
            "thunder",
            "_kf_",
        ...
        dfc = dfc[
            ~dfc["base_name"].str.contains("|".join(drop_files))
        ]  # (drop_files)])
        if search:
    ...
    def check_variable(self, file, search):
        ...
        if search:
            param = param[
                param.apply(lambda x: x.str.contains(search))
            ]  # .any(axis=1)]
    ...
    def check_files(self, date, model, param, mbrs, levtype):
        ...
        while i < len(df):
            ...
            # check leveltype
            df.loc[i, "mbr_bool"] = ("ensemble_member" in dimdic) and (
                dimdic["ensemble_member"] >= 10
            )
            df.loc[i, "ml_bool"] = ("hybrid" in dimdic) and (dimdic["hybrid"] >= 65)
            df["p_levels"] = df["p_levels"].astype(object)
            df.at[i, "p_levels"] = (
                [int(x) for x in dataset.variables["pressure"][:]]
                if "pressure" in dimdic and dimdic["pressure"] >= 1
                else None
            )
            # df.loc[i,"p_levels"] = ("pressure" in dimdic) and (dimdic["pressure"]>=10)
            av_pl_levels = (
                dataset.variables["pressure"][:] if "pressure" in dimdic else None
            )
            if "pressure" in dimdic:
                dimframe.loc["pressure", "value"] = ",".join(
                    str(int(x)) for x in av_pl_levels
                )
            ...
            dv_dim = [
                dataset.variables[d].dimensions for d in dv
            ]  # save var dimensions / what it depends on
        ...
        file_corrtype = filter_type(
            df.copy(), mbrs, levtype, self.p_level, self.m_level
        )
        ...
        if (
            ...
            SomeError(
                ValueError,
                f"File does noes not exist at date: {self.date} for model {self.model} and. Try again (You might want to split up the request). Available files are: \n {df}",
            )
        return file
