...
logging.basicConfig(
    filename="get_data.log", level=logging.INFO, format="%(levelname)s : %(message)s"
)
...
check_if_thredds_is_down = (
    lambda value: value
    if requests.head(value) != 200
    else SomeError(
        ConnectionError, f"Website {value} is down;. Wait until it is up again"
    )
)
filter_function_for_type = (
    lambda value: value
    if value in levtype
    else SomeError(ValueError, f"Type not found: choices:{levtype}")
)
filter_function_for_models = (
    lambda value: value
    if value in available_models
    else SomeError(ValueError, f"Model not found: choices:{model}")
)
filter_function_for_date = (
    lambda value: value
    if (len(value) == 10)
    and (int(value[0:4]) in range(2000, 2030))
    and (int(value[4:6]) in range(0, 13))
    and (int(value[6:8]) in range(1, 32))
    and (int(value[9:10]) in range(0, 25))
    else SomeError(
        ValueError, f'Modelrun wrong: Either; "latest or date on the form: YYYYMMDDHH'
    )
)
filter_function_for_mbrs = (
    lambda value, file: value
    if max(value) < file.dim["ensemble_member"]["shape"]
    else SomeError(ValueError, f"Member input outside range of model")
)
filter_function_for_step = (
    lambda value, file: value
    if np.max(value) < file.dim["time"]["shape"]
    else SomeError(ValueError, f" step input outside range of model")
)
filter_function_for_p_level = (
    lambda value, file: value
    if set(value).issubset(set(file["p_levels"]))
    else SomeError(ValueError, f" p_level input outside range of model")
)
filter_function_for_m_level = (
    lambda value, file: value
    if np.max(value) < file.dim["hybrid"]["shape"]
    else SomeError(ValueError, f" m_level input outside range of model")
)
filter_function_for_param = (
    lambda value, file: value
    if set(value).issubset(set(file["var"].keys()))
    else SomeError(ValueError, f" param input not possible for this file")
)
filter_function_for_file = (
    lambda value: value
    if value is not None
    else SomeError(ValueError, f" File has to be given (see check_data(()")
)
...
class get_data:
    def __init__(
        ...
        self.step = step
        self.p_level = (
            p_level if type(p_level) == list or p_level == None else list(p_level)
        )
        ...
        self.idx = (
            ...
            if data_domain
                ...
                np.array([0, self.file["var"]["y"]["shape"][0] - 1]),
                np.array([0, self.file["var"]["x"]["shape"][0] - 1]),
            )
        )
        ...
        check_if_thredds_is_down(
            "https://thredds.met.no/thredds/catalog/meps25epsarchive/catalog.html"
        )
        if self.url is None:
            filter_function_for_file(self.file)
            filter_function_for_models(self.model)
            filter_function_for_mbrs(
                np.array(self.mbrs), self.file
            ) if self.mbrs != None and self.mbrs_bool else None
            ...
            filter_function_for_p_level(
                np.array(self.p_level), self.file
            ) if self.p_level != None else None
            filter_function_for_m_level(
                np.array(self.m_level), self.file
            ) if self.m_level != None and self.file.ml_bool else None
            filter_function_for_param(
                self.param, self.file
            )  # this is kind of checked in check_data.py already.
    ...
    def adjust_user_url(self):
        if (
            ...
            and self.data_domain is None
            and self.p_level is None
            and self.m_level is None
            and self.mbrs is None
        ):
            return self.url
        ...
        if self.p_level:
            idx = np.where(
                np.array(self.file["p_levels"])[:, None]
                == np.array(self.p_level)[None, :]
            )[0]
        else:
            idx = 0
        ...
        self.param = np.append(
            self.param, fixed_var
        )  # Contains absolutely all variables we want
        ...
        for (
            ...
            param
        ):  # loop that updates the url to include each parameter with its dimensions
            url += f"{prm}"  # example:  url =url+x_wind_pl
            dimlist = list(
                file["var"][prm]["dim"]
            )  # List of the variables the param depends on ('time', 'pressure', 'ensemble_member', 'y', 'x')
            newlist = [
                indexidct[i] for i in dimlist
            ]  # convert dependent variable name to our set values. E.g: time = step = [0:1:0]
            startsub = (
                "".join(newlist) + ","
            )  # example: ('time', 'pressure','ensemble_member','y','x') = [0:1:0][0:1:1][0:1:10][0:1:798][0:1:978]
            for dimen in np.setdiff1d(file["var"][prm]["dim"], self.param):
                ...
                self.param = np.append(
                    self.param, dimen
                )  # update global param with the var name so that we do not go through it multiple time.
                startsub += dimen
    ...
    def make_url(self):
        ...
        if (
            self.model == "MEPS"
        ):  # This has become completely equal to AromeArctic except for base url. Consider adding them in the future.
            file = self.file.copy()
            param = (
                self.param.copy()
            )  # has to be copied in order for not infinite loop when updating self.param
            logging.info(file)
            ...
            for (
                ...
                startsub = (
                    "".join(newlist) + ","
                )  # example: ('time', 'pressure','ensemble_member','y','x') = x_wind_pl[0:1:0][0:1:1][0:1:10][0:1:798][0:1:978]
                for dimen in np.setdiff1d(file["var"][prm]["dim"], self.param):
                    # includes the dim parameters like, pressure, hybrid, height as long as we havent already gone through them
                    self.param = np.append(
                        self.param, dimen
    ...
    def thredds(self, url, file):
        ...
        for prm in self.param:
            ...
            dimlist = np.array(
                list(file["var"][prm]["dim"])
            )  # ('time', 'pressure', 'ensemble_member', 'y', 'x')
            if not self.mbrs_bool and any(
                np.isin(dimlist, "ensemble_member")
            ):  # "ensemble_member" in dimlist:
    ...
    def windcorr(self):
        ...
        self.__dict__["alpha"] = alpha[
            jindx.min() : jindx.max() + 1, iindx.min() : iindx.max() + 1
        ]
