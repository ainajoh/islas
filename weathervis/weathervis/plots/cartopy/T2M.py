    ...
    for dt in datetime:  # modelrun at time..
        ...
        for tim in np.arange(np.min(steps), np.max(steps) + 1, 1):
            ...
            if stepok == True:
                ...
                TA = np.where(
                    Z < 50000, tmap_meps.air_temperature_2m[tidx, :, :], np.NaN
                ).squeeze()
                MSLP = np.where(
                    Z < 50000,
                ...
                TA = np.where(
                    Z < 2000, tmap_meps.air_temperature_2m[tidx, :, :], np.NaN
                ).squeeze()
                C_T = ax1.contour(
                    dmap_meps.x,
                    dmap_meps.y,
                ...
                ax1.text(
                    0,
                    1,
                    "{0}_T2M_{1}+{2:02d}".format(model, dt, ttt),
                    ha="left",
                    va="bottom",
                ...
                print(
                    ...
                    + "/{0}_{1}_T2M_{2}+{3:02d}.png".format(model, domain_name, dt, tim)
                )
                fig1.savefig(
                    make_modelrun_folder
                    + "/{0}_{1}_T2M_{2}+{3:02d}.png".format(
