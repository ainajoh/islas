  ...
  for dt in datetime: #modelrun at time..
    ...
    for tim in np.arange(np.min(steps), np.max(steps)+1,1):
        ...
        for lev in levs:
          ...
          F_P = ax1.pcolormesh(lons, lats, spec2a,  norm=colors.LogNorm(vmin=1e-6, vmax=0.1), cmap=plt.cm.Reds, zorder=1, alpha=0.9, transform=ccrs.PlateCarree())
          F_P = ax1.pcolormesh(lons, lats, spec2b,  norm=colors.LogNorm(vmin=1e-6, vmax=0.1), cmap=plt.cm.Blues, zorder=2, alpha=0.9, transform=ccrs.PlateCarree())
          del spec2a
          del spec2b
