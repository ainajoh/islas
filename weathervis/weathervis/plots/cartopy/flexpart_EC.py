    ...
    for dt in datetime:  # modelrun at time..
        ...
        if not split:
            ...
            data_domain = domain_input_handler(
                dt, model, domain_name, domain_lonlat, file_all, use_latest=latest
            )
            ...
            data_domain = domain_input_handler(
                dt, model, domain_name, domain_lonlat, file_sfc, use_latest=latest
            )
            lonlat = np.array(data_domain.lonlat)
            dmap_meps = get_data(
                model=model,
        ...
        release_name = "NYA"
        cdf = nc.Dataset(
            "/home/centos/flexpart/{0}/grid_conc_{1}0000.nc".format(release_name, dt),
            "r",
        )
        ...
        for tim in np.arange(np.min(steps), np.max(steps) + 1, 1):
            ...
            if stepok == True:
                ...
                for lev in levs[0:4]:
                    ...
                    if tim <= 66:
                        MSLP = dmap_meps.air_pressure_at_sea_level[
                            tidx, 0, :, :
                        ].squeeze()
                    ...
                    if tim <= 66:
                        ...
                        ax1.clabel(
                            C_P, C_P.levels, inline=True, fmt="%3.0f", fontsize=10
                        )
