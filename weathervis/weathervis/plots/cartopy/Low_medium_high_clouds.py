    ...
    for dt in datetime:  # modelrun at time..
        ...
        for tim in np.arange(np.min(steps), np.max(steps) + 1, 1):
            ...
            if stepok == True:
                ...
                data[
                    np.where(data < 0.5)
                ] = np.nan  # take 0 and very small cloud covers away
                CCl = ax1.contourf(
                    x,
                    y,
                    data,
                ...
                ax1.text(
                    0,
                    1,
                    "{0}_CCLMH_{1}_+{2:02d}".format(model, dt, ttt),
                    ha="left",
                    va="bottom",
                ...
                print(
                    ...
                    + "/{0}_{1}_{2}_{3}+{4:02d}.png".format(
                        model, domain_name, "CCLMH", dt, ttt
                    )
                )
                grid = True
                if grid:
