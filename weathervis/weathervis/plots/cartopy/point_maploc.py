...
class MAP:
    def __init__(
        ...
        self.param_sfc = np.unique(
            ["x", "y", "latitude", "land_area_fraction", "surface_geopotential"]
            + param_sfc
        ).tolist()
    ...
    def retrieve_handler(self):
        ...
        if self.param_sfx:
            ...
            try:
                check_sfx = check_data(
                    date=self.date,
                    model=self.model,
                    param=self.param_sfx,
                    step=self.steps,
                )
            except ValueError:
    ...
    def plot_maplocation(
        ...
        for p in close2point:
            ...
            ax.background_patch.set_facecolor(
                "lightskyblue"
            ...
            ax.set_extent(
                (lonlat[0], lonlat[1], lonlat[2], lonlat[3])
            )  # (x0, x1, y0, y1)
            ...
            ax.legend(
                (all_gridpoint, mainpoint, closep, CC.collections[0]),
                (
                    "Gridpoints",
                    f"Point of interest({point_lonlat[1]}N{point_lonlat[0]}E)",
                    "Analysed gridpoints",
                    "model coastline",
                ),
                loc="upper right",
...
if __name__ == "__main__":
    ...
    parser.add_argument("--point_name", default=None, nargs="+", help="see sites.yaml")
    parser.add_argument(
    ...
    for dt in args.datetime:
        (
            ...
            figname_b2,
            figname_b3,
        ) = setup_directory(dt, args.point_name, args.point_lonlat)
