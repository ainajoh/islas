...
def Wind_on_levels(
    ...
    for dt in datetime:  # modelrun at time..
        ...
        cm = colors.ListedColormap(
            ["#f4f8f8", "#98d1f0", "#f9db65", "#f45510", "#c22ecb", "#531357"]
        )
        ...
        for ip, p in enumerate(dmet.pressure):
            ...
            for tim in np.arange(np.min(steps), np.max(steps) + 1, 1):
                ...
                if stepok == True:
                    ...
                    # calculate windspeed
                    WS = np.sqrt(
                        dmet.x_wind_pl[tidx, ip, :, :] ** 2
                        + dmet.y_wind_pl[tidx, ip, :, :] ** 2
                    )
                    ...
                    #########################
                    CC = ax1.contourf(
                        x, y, WS, levels=np.linspace(0.0, 30, 7), cmap=cm, zorder=2
                    )
                    ...
                    ax1.text(
                        0,
                        1,
                        "{0}_WS_{1}_{2}+{3:02d}".format(model, int(p), dt, ttt),
                        ha="left",
                        va="bottom",
                    ...
                    print(
                        ...
                        + "/{0}_{1}_{2}_{3}_{4}+{5:02d}.png".format(
                            model, domain_name, "WS", int(p), dt, ttt
                        )
                    )
                    grid = True
                    if grid:
                    ...
                    if legend:
                        ...
                        proxy = [
                            plt.axhline(y=0, xmin=0, xmax=0, color="gray", zorder=7)
                        ]
                        # proxy.extend(proxy1)
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        nargs="+",
        type=int,
        help="p_level example --p_level 1000 925",
    )
