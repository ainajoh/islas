    ...
    for dt in datetime:  # modelrun at time..
        ...
        try:
            ...
            try:
                ...
                check_pl = check_data(
                    date=dt, model=model, param=param_pl, p_level=850, step=steps
                )
            except ValueError:
                print(
        ...
        for tim in np.arange(np.min(steps), np.max(steps) + 1, 1):
            ...
            if stepok == True:
                ...
                W = np.where(
                    Z < 3000, tmap_meps.upward_air_velocity_pl[tidx, plev, :, :], np.NaN
                ).squeeze()
                MSLP = np.where(
                    Z < 3000, dmap_meps.air_pressure_at_sea_level[tidx, 0, :, :], np.NaN
                ).squeeze()
                BLH = (
                    tmap_meps.atmosphere_boundary_layer_thickness[tidx, :, :]
                ).squeeze()
                ...
                ax1.text(
                    0,
                    1,
                    "{0}_BLH_{1}+{2:02d}".format(model, dt, ttt),
                    ha="left",
                    va="bottom",
                ...
                if legend:
                    ...
                    proxy = [
                        ...
                        plt.axhline(
                            y=0, xmin=1, xmax=1, color="blue", linestyle="dashed"
                        ),
