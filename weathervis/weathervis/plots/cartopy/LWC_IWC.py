...
def IWC_LWC(datetime, steps=0, model= "MEPS", domain_name = None, domain_lonlat = None, legend=False, info = False,grid=True,m_level = [0, 64], runid=None, outpath=None):
    ...
    for dt in datetime:  # modelrun at time..
        ...
        param = ['mass_fraction_of_cloud_condensed_water_in_air_ml','surface_air_pressure',
        ...
        #print(m_level)
        dmet = get_data(model=model, data_domain=data_domain, param=param, file=file_all, step=steps,
        ...
        for tim in np.arange(np.min(steps), np.max(steps)+1, 1):
            ...
            if stepok==True:
                ...
                # calculate LWC
                LWC=dmet.mass_fraction_of_cloud_condensed_water_in_air_ml[tidx,:,:,:]
                ...
                MSLP = np.where(ZS < 3000, dmet.air_pressure_at_sea_level[tidx, 0, :, :], np.NaN).squeeze()
                ...
                CC=ax1.pcolormesh(x, y,  ldata[:, :], cmap=plt.cm.Reds, vmin=0.002, vmax=0.1,zorder=2)
                IWC[np.where(IWC > 0.005)] = np.nan
                CI= ax1.pcolormesh(x, y, idata[:, :], cmap=plt.cm.Blues,alpha=0.3, vmin=0.0005, vmax=0.04,zorder=3)
                ...
                ax1.text(0, 1, "{0}_LWC_IWC_{1}_{2}+{3:02d}".format(model,m_level, dt, tim), ha='left', va='bottom', \
                ...
                print("filename: "+make_modelrun_folder + "/{0}_{1}_{2}_{3}+{4:02d}.png".format(model, domain_name, "clouds", dt, tim))
                fig1.savefig(make_modelrun_folder + "/{0}_{1}_clouds_{2}+{3:02d}.png".format(model, domain_name, dt, tim), bbox_inches="tight", dpi=200)
                ax1.cla()
                plt.clf()
                plt.close(fig1)
        #ax1.cla()
