...
def IWC_LWC(
    ...
    for dt in datetime:  # modelrun at time..
        ...
        # CALCULATE
        dmap_meps.LWC = np.sum(
            dmap_meps.mass_fraction_of_cloud_condensed_water_in_air_ml[:, :, :, :],
            axis=1,
        )
        dmap_meps.LWC = dmap_meps.LWC * 1000
        dmap_meps.units.LWC = "g/kg"
        dmap_meps.IWC = np.sum(
            dmap_meps.mass_fraction_of_cloud_ice_in_air_ml[:, :, :, :], axis=1
        )
        ...
        for tim in np.arange(np.min(steps), np.max(steps) + 1, 1):
            ...
            if stepok == True:
                ...
                data[mask] = np.nan
                CC = ax1.pcolormesh(
                    x, y, data[:, :], cmap=plt.cm.Reds, vmin=0.1, vmax=4.0, zorder=2
                )
                ...
                ax1.text(
                    0,
                    1,
                    "{0}_LWC_IWC_{1}_{2}+{3:02d}".format(model, m_level, dt, ttt),
                    ha="left",
                    va="bottom",
                ...
                print(
                    ...
                    + "/{0}_{1}_{2}_{3}+{4:02d}.png".format(
                        model, domain_name, "LWP_IWP", dt, ttt
                    )
                )
                if grid:
                    nicegrid(ax=ax1)
                ...
                if legend:
                    cbar = nice_vprof_colorbar(
                        ...
                        height=0.05,
                        format="%.3f",
                        ticks=[0.001, np.nanmax(dmap_meps.IWC[tidx, :, :]) * 0.8],
                    )
                    cbar = nice_vprof_colorbar(
                        CF=CC,
                        ax=ax1,
                        ...
                        height=0.05,
                        format="%.1f",
                        ticks=[0.09, np.nanmax(dmap_meps.LWC[tidx, :, :]) * 0.8],
                    )
