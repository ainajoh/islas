...
def plot_meteogram_vertical(
    ...
    if np.min(dmet.latitude) > 69:
        ...
        lvl = np.linspace(
            np.min(specific_humidity_ml[:, :, jindx, iindx]),
            np.max(specific_humidity_ml[:, :, jindx, iindx]),
            20,
        )
    ...
    CF_Q = axm1.contourf(
        tx,
        p[:, :, jindx, iindx],
        specific_humidity_ml[:, :, jindx, iindx],
        levels=lvl,
        cmap=cmap,
        extend="both",
        zorder=1,
    ...
    axm1.legend(
        [custom_lines[0], custom_lines[1], custom_lines[2]],
    ...
    n = 4
    wspeed = np.sqrt(
        dmet.x_wind_ml[:, :, jindx, iindx] ** 2
        + dmet.y_wind_ml[:, :, jindx, iindx] ** 2
    )
    cmap = plt.cm.RdYlBu_r  # plt.cm.jet RdYlBu

    ti = np.arange(
        0, dmet.x_wind_ml.shape[0], 4
    )  # skips over every 100th value 100 = inbetween
    ...
    axm2.barbs(
        tx[points],
        p_p[points],
        uml_p[points] * 1.943844,
        vml_p[points] * 1.943844,
        length=5,
        zorder=1000,
    )
...
def plot_meteogram(dmet, dmet_sfx, dmet_ml, jindx, iindx, dirName_b0, figname_b0, ip):
    ...
    S_bar = axm2_3.bar(
        dmet.time_normal[0::3],
        dmet_ml.sample_ml[0::3, -1, jindx, iindx],
        ls="--",
        ec="green",
        color="lightblue",
        alpha=0.8,
    ...
    # label
    axm2.legend(
        [Q2M[0], QML0[0], S_bar[0]],
        ["2m Spec.Hum.", "ml0 Spec.Hum.", "3h acc samplesize"],
        loc="upper left",
    ).set_zorder(99999)
    ...
    # pressure
    axm3_2.plot(
        dmet.time_normal, dmet.surface_air_pressure[:, 0, jindx, iindx] / 100, color="k"
    )
    ...
    axm4.legend(
        [P_SH[0], P_LH[0]], ["Sensible Heat Flux", "Latent Heat Flux"], loc="upper left"
    ).set_zorder(99999)
...
def plot_maplocation(
    ...
    crs = ccrs.LambertConformal(
        central_longitude=lon0, central_latitude=lat0, standard_parallels=parallels
    )
...
def meteogram_average(dmet, dmet_sfx, dmet_ml, indx, dirName_b2, figname_b2, sitename):
    ...
    axma2_3.legend(
        [Q2M_mean[0], S_bar[0]],
        ["2m mean Spec.Hum..", "mean 3h acc samplesize"],
        loc="upper left",
    ).set_zorder(99999)
    ...
    axma3.legend(
        [GUST_MEAN[0], WIND_MEAN[0]],
        ["10m wind gust", "10m wind (10min mean)"],
        loc="upper left",
    ).set_zorder(99999)
