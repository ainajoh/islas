        ...
        for tim in np.arange(np.min(steps), np.max(steps) + 1, 1):
            ...
            if stepok == True:
                ...
                ax1.text(
                    0,
                    1,
                    "{0}_dxs_{1}+{2:02d}".format(model, dt, ttt),
                    ha="left",
                    va="bottom",
                ...
                if legend:
                    proxy = [
                        plt.axhline(y=0, xmin=1, xmax=1, color="tab:blue"),
                        plt.axhline(
                            y=0, xmin=1, xmax=1, color="dimgrey", linewidth=0.5
                        ),
                        plt.axhline(y=0, xmin=1, xmax=1, color="black", linestyle="--"),
                    ]
                    lg = plt.legend(
                        proxy, ["d-xs=10", "MSLP [hPa]", "Sea Ice conc. >10%"], loc=1
                    )
                ...
                print(
                    ...
                    + "/{0}_{1}_dxs_{2}+{3:02d}.png".format(model, domain_name, dt, ttt)
                )
                if grid:
                    nicegrid(ax=ax1)
                ...
                fig1.savefig(
                    make_modelrun_folder
                    + "/{0}_{1}_dxs_{2}+{3:02d}.png".format(
