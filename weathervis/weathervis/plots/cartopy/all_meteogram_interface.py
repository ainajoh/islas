...
from point_vertical_metegram import *
...
class LOC_PLOTS:
    ...
    def points(self, dmet):
        ...
        if point_lonlat:
            ...
            print(sites.loc[self.point_name].lon)
...
def input_handler(date, steps, model, domain_name,domain_lonlat, legend, info, num_point,point_lonlat, point_name):
    ...
    PM = PMET(date=date, steps=steps, model=model, domain_name=domain_name,
              domain_lonlat=domain_lonlat, legend=legend, info=info, num_point=num_point,
              point_lonlat=point_lonlat, point_name=point_name)
    ...
    VM = VERT_MET(date=date, steps=steps, model=model, domain_name=domain_name,
              domain_lonlat=domain_lonlat, legend=legend, info=info, num_point=num_point,
              point_lonlat=point_lonlat, point_name=point_name)
    ...
    LP = LOC_PLOTS(date=date, steps=steps, model=model, domain_name=domain_name,
              domain_lonlat=domain_lonlat, legend=legend, info=info, num_point=num_point,
              point_lonlat=point_lonlat, point_name=point_name,param_pl=param_pl, param_ml =param_ml,
                   param_sfc =param_sfc, param_sfx=param_sfx)
    ...
    LP.dmet = dmet
    points, indx_sea, indx_land, indx_alldomain, index_neares,point_lonlat= LP.points(dmet)
    ...
    for av in averagesite:
        ...
        M.plot_maplocation(indx, dirName_b2, figname_b2, sitename=av, point_lonlat=point_lonlat, all=True)
...
if __name__ == "__main__":
    ...
    for dt in args.datetime:
        #print(args.datetime)
        input_handler(date=dt, steps=args.steps, model=args.model, domain_name=args.domain_name,
