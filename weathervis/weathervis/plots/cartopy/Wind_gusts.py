...
def Wind_gusts(datetime, steps, model, domain_name = None, domain_lonlat = None, legend=False, info = False,grid=True, runid=None, outpath=None):
    global OUTPUTPATH
    if outpath != None:
        OUTPUTPATH=outpath
    ...
    for dt in datetime: #modelrun at time..
        ...
        for tim in np.arange(np.min(steps), np.max(steps)+1, 1):
            ...
            if stepok==True:
                ...
                xG  = dmet.x_wind_gust_10m[tidx,0,:,:].squeeze()
                yG  = dmet.y_wind_gust_10m[tidx,0,:,:].squeeze()
                WSG = np.sqrt(xG**2+yG**2)
                # mean wind
                xW  = dmet.x_wind_10m[tidx,0,:,:].squeeze()
                yW  = dmet.y_wind_10m[tidx,0,:,:].squeeze()
                WSW = np.sqrt(xW**2+yW**2)
                Wd = WSG-WSW
                # plotting mean wind speed value
                C_WS=ax1.contourf(x,y,WSW,levels=np.linspace(0.0, 30, 13),
                                 cmap=plt.cm.jet,alpha=0.9,zorder=2)
                ...
                print("filename: "+make_modelrun_folder +"/{0}_{1}_{2}_{3}+{4:02d}.png".format(model, domain_name, "WG10", dt, ttt))
                grid = True
                if grid:
                    nicegrid(ax=ax1)
...
if __name__ == "__main__":
  ...
  Wind_gusts(datetime=args.datetime, steps = [np.min(args.steps), np.max(args.steps)],
                 model = args.model,domain_name = args.domain_name, domain_lonlat=args.domain_lonlat,
