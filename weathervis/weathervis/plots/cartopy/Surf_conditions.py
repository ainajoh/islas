# %%
# python Surf_conditions.py --datetime 2020091000 --steps 0 1 --model MEPS --domain_name West_Norway
    ...
    for dt in datetime:  # modelrun at time..
        ...
        try:
            ...
            try:
                ...
                check_sfx = check_data(
                    date=dt, model=model, param=param_sfx, step=steps
                )
            except ValueError:
                print(
        ...
        for tim in np.arange(np.min(steps), np.max(steps) + 1, 1):
            ...
            if stepok == True:
                ...
                MSLP = np.where(
                    ZS < 50, dmap_meps.air_pressure_at_sea_level[tidx, 0, :, :], np.NaN
                ).squeeze()
