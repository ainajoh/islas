...
cp = 1005.0  # specific heat of dry air, constant pressure [J K^-1 kg^-1]
cv = 718.0  # sepcific heat of dry air, constant volume J K^-1 kg^-1
...
class VERT_CROSS:
    ...
    def crosssection_as_meteogramm(
        self,
        date,
        dirName,
        timest,
        ...
        for cc, kk, st, ed, fnam in zip(coos2, K, stnam, endnam, fignam):
            ...
            dtdz2 = (
                cross.TT[1::, :] - cross.TT[0:-1, :]
            ) * 10  # C/ km, *10 due to the interpolated height intervals of 100m
            # for the CAF hashing
            ...
            cbar = fig.colorbar(pw, cax=cbar_ax, orientation="vertical")
            cbar.ax.tick_params(labelsize=15)
            ...
            plt.savefig(
                dirName + "VCS_wind_" + fnam + "_{0}+{1:02d}.png".format(date, timest),
                bbox_inches="tight",
                dpi=200,
            )
            print(
                "Saved crosssection as {}".format(
                    dirName
                    + "VCS_wind_"
                    + fnam
                    + "_{0}+{1:02d}.png".format(date, timest)
                )
            )
            ...
            rr = ax2.contourf(x, zi, prec, cmap="cool", extend="both")
            ...
            ax2.text(x[1, -50], -520, ed, fontsize=15)
            cbar_ax = fig.add_axes([0.91, 0.563, 0.015, 0.3])
            cbar = fig.colorbar(pc, cax=cbar_ax, orientation="vertical", extend="both")
            cbar.ax.tick_params(labelsize=15)
            cbar.ax.set_ylabel("lapse rate [°C/km]", fontsize=15)
            cbar_rr = fig.add_axes([0.7, 0.845, 0.18, 0.015])
            cbar2 = fig.colorbar(rr, cax=cbar_rr, orientation="horizontal")
            ...
            cbarW = fig.colorbar(cs2, cax=cbarW, orientation="horizontal")
            ...
            plt.savefig(
                dirName + "VCS_cloud_" + fnam + "_{0}+{1:02d}.png".format(date, timest),
                bbox_inches="tight",
                dpi=200,
            )
            print(
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--domain_lonlat",
        default=[-4.770555, 41.700291, 64.6152131, 79.477055],
        nargs="+",
    ...
    parser.add_argument(
        "--extra_names",
        default=None,
        help="give list of names for additional crossection",
    )
    parser.add_argument(
        "--extra_coords", default=None, help="give list of extra coordinates"
    )
    ...
    parser.add_argument(
        "--start_name", default="NYA", help="name of start location", type=str
    )
    parser.add_argument(
        "--end_name", default="KRN", help="name of end location", type=str
    )
    parser.add_argument(
        "--m_level",
        default=[0, 64],
    ...
    for dt in args.datetime:
        ...
        for st in np.arange(
            args.steps[0], args.steps[1], 6
        ):  # for now assuming that we start at 0
            ...
            VC.crosssection_as_meteogramm(
                date=dt,
                dirName=dirName,
                timest=st,
                end_nam=args.end_name,
                st_nam=args.start_name,
                coords=args.points_lonlat,
                orient=args.orient,
            )
