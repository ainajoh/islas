...
#modelrun=("2020031512")
#modelrun=("2020101012")
point_name=("Andenes" "Tromso" "NyAlesund" "NorwegianSea" "Bjornoya" "CAO" "Longyearbyen")
for md in ${model[@]}; do
  echo $md
  for ((i = 0; i < ${#modelrun[@]}; ++i)); do
      for pnam in ${point_name[@]}; do
	 runstring_Pmet="python point_meteogram.py --datetime ${modelrun[i]} --steps 0 ${steps_max[i]} --model $md --domain_name $pnam --point_name $pnam"
