...
def flexpart_EC(datetime, steps=0, model= "MEPS", domain_name = None, domain_lonlat = None, legend=False, info = False, save = True, grid=True):
  ...
  for dt in datetime: #modelrun at time..
    ...
    for release_name in all_release_name:
        path="/Data/gfi/work/cat010/flexpart_arome/output/{0}/{1}*/flexpart_run_d01_combined.nc".format(dt,release_name)
        ...
        if stepok==True:
            ...
            for lev in levs[0:last_lvl_idx_for_plotting+1]:
              ...
              for i in range(0,len(spec)-1):
                  ...
                  #F_P = ax1.pcolormesh(x, y, ss,  norm=colors.LogNorm(vmin=1e-10, vmax=0.2), cmap=my_colors[i], zorder=1, alpha=0.9)
