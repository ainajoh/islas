...
class VERT_MET:
    ...
    def vertical_met(self, jindx, iindx, dirName_b1, figname_b1, ip, p_top=500):
        ...
        dmet.mass_fraction_of_cloud_condensed_water_in_air_ml = (
            dmet.mass_fraction_of_cloud_condensed_water_in_air_ml * 1000.0 * 1000
        )  # g/kg
        dmet.mass_fraction_of_cloud_ice_in_air_ml = (
            dmet.mass_fraction_of_cloud_ice_in_air_ml * 1000.0 * 1000
        )
        # MASK
        ...
        areafrac_cloud = np.ma.array(
            dmet.cloud_area_fraction_ml[:, :, jindx, iindx], mask=p_p < p_top
        )
        # mass_fraction_of_snow_in_air_ml = dmet.mass_fraction_of_snow_in_air_ml[:, :, jindx, iindx]
        lvls = [0]
        mass_fraction_of_cloud_condensed_water_in_air_ml = np.ma.array(
            dmet.mass_fraction_of_cloud_condensed_water_in_air_ml[:, :, jindx, iindx],
            mask=p_p < p_top,
        )
        mass_fraction_of_cloud_ice_in_air_ml = np.ma.array(
            dmet.mass_fraction_of_cloud_ice_in_air_ml[:, :, jindx, iindx],
            mask=p_p < p_top,
        )
        ...
        # INITIALISING
        figm1, (axm1, axm2) = plt.subplots(
            nrows=2, ncols=1, figsize=(12, 14), sharex=False
        )
        ...
        CF_Q = axm1.contourf(
            tx, p_p, q_p, levels=lvl, cmap=cmap, extend="both", zorder=1
        )
        ...
        lvl = np.linspace(
            np.min(mass_fraction_of_cloud_ice_in_air_ml),
        ...
        axm1.clabel(
            CF_C, CF_C.levels, inline=False, fmt="%3.1f", fontsize=12, colors="gray"
        )
        ...
        axm2.barbs(
            tx[skip][skip],
            p_p[skip][skip],
            ur_p[skip][skip] * 1.943844,
            vr_p[skip][skip] * 1.943844,
            length=7,
            zorder=1000,
        ...
        # potential temp.
        lvl = np.arange(
            np.min(dmet.theta[:, :, jindx, iindx]),
            np.max(dmet.theta[:, :, jindx, iindx]),
            2,
        )
        ...
        cfrac_leg = axm2.legend(
            artists_C, ["Potential temp."], handleheight=2, loc="upper left"
        ).set_zorder(99999)
        ...
        axm1.text(
            0,
            1,
            "{0}_VPMET_{1}_{2}".format(self.model, self.point_name, dt),
            ha="left",
            va="bottom",
            transform=axm1.transAxes,
            color="black",
        )
        axm2.text(
            0,
            ...
            ha="left",
            va="bottom",
            transform=axm2.transAxes,
            color="black",
        )

        plt.savefig(
            dirName_b1 + figname_b1 + "_op2" + ".png", bbox_inches="tight", dpi=200
        )
        ...
        cbar = nice_vprof_colorbar(
            CF=CF_2, ax=axm2, label="Temp. [K]", extend="both", format="%.0f"
        )
        ...
        plt.savefig(
            dirName_b1 + figname_b1 + "_op1" + ".png", bbox_inches="tight", dpi=200
        )
