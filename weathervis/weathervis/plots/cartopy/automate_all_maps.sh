...
while [ $# -gt 0 ]; do
   ...
   for md in ${model[@]}; do
      ...
      for ((i = 0; i < ${#modelrun[@]}; ++i)); do
         ...
         runstring_OLR="python OLR_sat.py --datetime ${modelrun[i]} --steps 0 $steps_max  --model $md --domain_name $domain_name --id $id" runstring_BLH="python BLH.py --datetime ${modelrun[i]} --steps 0 $steps_max --model $md --domain_name $domain_name --id $id"
