...
for ((i=0;i<${#modelrun[@]};++i)); do
    ...
    runstring_Z="python Z500_VEL.py --datetime ${modelrun[i]} --steps 0 24 --model $md"
    runstring_CAO="python CAO_index.py --datetime ${modelrun[i]} --steps 2 24 --model $md"
    #runstring_SURF="python Surf_conditions.py --datetime ${modelrun[i]} --steps 0 ${steps_max[i]} --model $md --domain_name $domain_name"
    runstring_TOA="python TOA_sat.py --datetime ${modelrun[i]} --steps 2 24  --model $md"
