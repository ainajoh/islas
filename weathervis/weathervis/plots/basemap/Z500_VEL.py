  ...
  for dt in datetime: #modelrun at time..
    ...
    for tim in np.arange(np.min(steps), np.max(steps)+1, 1):
      ...
      try:  # workaround for a stupid matplotlib error not handling when all values are outside of range in lvl or all just nans..
        ...
        CF_prec = ax1.contourf(x, y, TP, zorder=10,cmap=cmap, norm=norm, alpha=0.4, antialiased=True, levels=lvl, extend="max")  #
      except:
        pass
      ...
      CS = ax1.contour(x, y, VEL, zorder=3, alpha=1.0,
                         levels=np.arange(-80, 80, 5), colors="green", linewidths=0.7)
      # geopotential
      CS = ax1.contour(x, y, Z, zorder=3, alpha=1.0,
    ...
    ##########################################################
      ...
      if info:
        ...
        #plt.show()
      fig1.savefig("../../../output/{0}_Z500_VEL_P_{1}+{2:02d}.png".format(model, dt, tim), bbox_inches="tight", dpi=200)
      ax1.cla()
    ...
    fig2.legend(proxy, [f"Wind strength [m/s] at {dmap_meps.pressure[plev2]:.0f} hPa",
