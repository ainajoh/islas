...
def find_best_combinationoffiles(
    all_param, fileobj, m_level=None, p_level=None
):  # how many ways ca we split up all the possible balls between these kids?
    ...
    for i in range(0, len(fileobj)):
        ...
        param_we_want_that_are_available = [
            x for x in param_available if x in all_param
        ]
        param_we_want_that_areNOT_available = [
            x for x in all_param if x not in param_we_want_that_are_available
        ]
    ...
    def filer_param_by_modellevels(
        config_overrides_r, tot_param_we_want_that_are_available
    ):
        print(len(fileobj))
        for i in range(0, len(fileobj)):
            ...
            f = pandas_df[
                pandas_df.dim.astype(str).str.contains("hybrid")
            ]  # keep only ml variables.

            if len(f) != 0:  # if var depends on hybrid.
                dimen = [
                    f.apply(lambda row: dict(zip(row["dim"], row["shape"])), axis=1)
                ][
                    0
                ]  # .loc["dim"]
                dimofmodellevel = [
                    dimen.apply(
                        lambda row: [
                            ...
                            if "hybrid" in key.lower()
                        ]
                    )
                ][
                    0
                ]  # .loc["dim
                removethese = dimofmodellevel[
                    dimofmodellevel.apply(lambda row: row[0] < m_level)
                ]  # .loc["dim"]
                val = [*removethese.index]
                key = thisfileobj["File"]
                config_overrides_r[key] = [
                    x for x in config_overrides_r[key] if x not in val
                ]

            return config_overrides_r

    config_overrides_r = filer_param_by_modellevels(
        config_overrides_r, tot_param_we_want_that_are_available
    )
    ...
    our_choice = ppd.loc[
        0
    ]  # The best combination retrieving from least amount of files.
    print(ppd)
...
def retrievenow(
    ...
    fixed_var = [
        ...
        "hybrid2",
        "hybrid0",
    ]  # this should be gotten from get_data
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--datetime", help="YYYYMMDDHH for modelrun", default=None, nargs="+"
    )
    parser.add_argument(
        "--steps",
        default=[0, 10],
    ...
    fileobj = check_data(
        args.model, date=str(args.datetime[0]), step=args.steps, use_latest=use_latest
    ).file
    ...
    for i in range(0, len(all_choices)):
        try:
            dmet = retrievenow(
                all_choices.loc[i], args.model, args.steps, str(args.datetime[0])
            )
            break
        except:
            # del(dmet)
